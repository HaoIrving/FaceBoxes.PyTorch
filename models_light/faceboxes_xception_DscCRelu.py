...
class DscCRelu(nn.Module):
  '''Depthwise conv + Pointwise conv from MobileNet v1'''
  def __init__(self, in_channels, out_channels, **kwargs):
    super(DscCRelu, self).__init__()
    # self.conv = nn.Conv2d(in_channels, out_channels, bias=False, **kwargs)
    # self.bn = nn.BatchNorm2d(out_channels, eps=1e-5)
...
class FaceBoxes(nn.Module):
  ...
  def __init__(self, phase, size, num_classes):
    ...
    self.conv1 = DscCRelu(3, 24, kernel_size=7, stride=4, padding=3)
    self.conv2 = DscCRelu(48, 64, kernel_size=5, stride=2, padding=2)
